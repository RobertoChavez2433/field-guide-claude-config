...
- Photo detail dialog (`lib/features/entries/presentation/widgets/photo_detail_dialog.dart`) for full-size viewing with metadata
- Local file storage with cloud sync of file and metadata

## Data Model
- Primary entity: `photos` (SQLite table)
- Key fields: `id` (UUID), `entry_id` (FK to daily_entries), `project_id` (FK to projects), `file_path` (nullable local device path — null when the photo exists remotely but has not yet been downloaded to this device), `filename`, `remote_path` (cloud URL, null until synced), `notes`, `caption`, `location_id` (optional FK to locations), `latitude`, `longitude`, `captured_at`, `sync_status` (pending/synced/error) (DEPRECATED — sync now uses `change_log` triggers, not per-row sync_status), `deleted_at` (nullable timestamp, set on soft-delete), `deleted_by` (nullable UUID, user who performed the soft-delete), `created_at`, `updated_at`
- Foreign keys: `entry_id` -> `daily_entries(id)` CASCADE, `project_id` -> `projects(id)` CASCADE, `location_id` -> `locations(id)` SET NULL
- Indexes: `idx_photos_entry`, `idx_photos_project`, `idx_photos_sync_status` (DEPRECATED)
- Sync: Sync to Cloud -- photo metadata syncs via `change_log` triggers; binary file uploads to Supabase Storage separately
...
## Offline Behavior
Full read/write offline. Photos are saved to local device storage immediately. Metadata is written to SQLite and captured by `change_log` triggers for sync when connectivity returns. File path is always local-first. When connectivity returns, the sync feature uploads the binary file to Supabase Storage and updates `remote_path`. If the device runs low on storage, photos are the largest consumer -- no automatic cleanup is performed; the user manages storage manually.
